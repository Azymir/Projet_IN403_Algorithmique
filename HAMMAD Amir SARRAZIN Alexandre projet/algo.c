...
int * calcule_parcours(graphe * G, int depart, int arrivee, enum t_skieur niveau) {
	...
	*(trace++) = depart;
	...
	printf("La durée de votre parcours est estimé à : %d \n", durees[arrivee-1]);
...
void afficher_parcours(int * parcours, graphe * G) {
...
int main(int argc, char **argv) {
	...
	if ((a != 0) && (a != 1)) {
	  printf("vous n'avez pas entre(e) le niveau correct");
	  exit(99);
	}
